...
\end{figure}
%
We define $w(Q)$ as a function, which returns the \texttt{WHERE} clause of the \texttt{SELECT} query $Q$.
%
Further we also define $\operatorname{freeV}(Q)$ as a function, which returns the set containing all free variables in the \texttt{SELECT} query $Q$.
%
...
\infer[\text{PROJ1}]
	{Q_1' \subseteq Q_2'}
	{Q_1 \subseteq Q_2 \qquad A_1 \setminus A_1' = A_2 \setminus A_2' \qquad 
		\lbrace	A_1, A_1', A_2, A_2' \rbrace \subseteq \operatorname{freeV}(Q_1) \cup \operatorname{freeV}(Q_2)}
\]
...
\]
\remark{Is it ok like this with the definition above?}
