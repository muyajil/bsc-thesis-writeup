...
%
The rules were developed for the relational algebra \remark{Relational calculus}, which as mentioned before differs from SQL. 
%
Here we extend these rules to SQL.
%
In the following we present this extended rule set. \remark{Remove this sentence. It just repeats the one before.}
%
Let $T_i$ denote \remark{Sets of?} tables, $A_i$ sets of attributes within tables, $Q_i$ \texttt{SELECT} queries of the form \texttt{SELECT $A$ FROM $T$ WHERE $\phi$, and greek letters stand for boolean expressions on the tuples contained in the tables.
%
The inference rules consist of tuples $(u,Q)$ where $u$ is the user issuing the query $Q$.
%
All the following rules are only applicable if the user $u$ is allowed to read all tables or views referenced in the premiss \remark{premise}.
%
\remark{Containment is independent from the user. I don't see why you added this restriction. It's probably even too restrictive for our algorithm.}


\remark{The rules are written in a very weird way. Who is contained in what? Look at how I've wrote the rules in the technical report.} 

\remark{Put all the rules in a figure. Give them a name. Explain each rule and refer to it as ID-rule, AND-rule, etc.}
...
\remark{In principle, we need also a rule for the $\psi$'s case.}

The rule holds since we weaken the condition by reducing the number of boolean expressions in the conjunction \remark{It's a bit unclear}. Therefore the tuples resulting from the query in the premiss are contained in the tuples resulting from the query in the conclusion.
...
\remark{Description is completely unclear. Moreover, the containment does not hold. 
%
Containment $Q_1' \subseteq Q_2'$ holds in case we already know that $Q_1 \subseteq Q_2$ and the queries $Q_1'$ and $Q_2'$ are obtained from $Q_1$ and $Q_2$ by projecting out the same attribute $A$.
}
...
\remark{It is not obvious at all. The containment again does not hold. THere is no tuple in the first query that is contained also in the second query.}
...
In the new algorithm one component of authorizing \texttt{SELECT} queries is the rewriting of those. \remark{Unclear}
%
The rewriting produces a new query, where the resulting set is the set of tuples equal to subtracting an under-approximation of the query from an over-approximation of the query. \remark{Unclear. Under- and Over- approximations with respect to what? Containment? These concepts have never been defined before. In SQL the result is a bag (or multiset) not a set.}
%
...
%
The approximations are computed based on the inference rules introduced above. \remark{No. The approximations are computed by the rewriting algorithm. The inference rules for containment are used only for computing the 
%
...
%
Further we define two functions $\operatorname{under}(Q)$ and $\operatorname{over}(Q)$,  where $Q$ is passed from $\operatorname{rewrite}$. \remark{Unclear. What is \texttt{operatorname}?}
%
These two functions return a query that is an over- respectively under-approximation. \remark{Again. With respect to what?}

\remark{where are the function?}

\smallskip
\noindent
{$\mathbf{ \mathrm{\bf over}(Q)}$:}
%
In this function we first build an inference forest that includes all possible inference trees from the rules introduced above. \remark{undefined concepts}
%
\remark{First give a precise algorithm. Afterwards, explain the algorithm with the example.}
...
In the example previously introduced a motivating example \remark{In Chapter X}, we showed that the current access control mechanism implemented in PostgreSQL \remark{is }too restrictive to be used, in practice, with powerful access control mechanisms such as the one in the paper.
%
...
%
$Q$ will be authorized if the result is \texttt{NULL}. \remark{I guess the result is $\emptyset$, not null.}

\remark{Why is this a good criterion for security? What does this section add to the story?}
