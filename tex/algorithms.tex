...
As shown in~\cite{guarnieri2014optimal}, query containment is a fundamental problem in database security. \remark{I'm pretty sure I don't show this in the VLDB paper. Why don't you just give some high-level overview of (1) query security, and (2) how the mechanism in \cite{guarnieri2016strong} works. Then, you can just say that you generalize the main components of \cite{guarnieri2016strong} to SQL. (Avoid just repeating the intro and background though).}
%
...
%
\remark{We use query containment to compute the approximations. However, the access control decision depends on the actual database state, and therefore is not related to query containment, which quantifies over all database states.}
...
%
Bags however, support the same operations as sets, in particular they support $\cap, \cup, \setminus$. \remark{Spell these operations out, don't use symbols}
%
SQL therefore allows to apply these operation to queries.
%
The rules INTER1 and INTER2 hold \remark{vs ARE SOUND} since we know that the intersection of two bags contains all elements that appear in both bags.
%
Therefore the intersection must be a subset of both initial bags.
%
The union of two bags contains all elements that appear in at least one of the intial \remark{typo} bags, therefore we can follow that both inital \remark{typo} bags are contained in the union.
%
The rule EXCEPT holds by the definition of subtracting multisets. \remark{You use bags 'til now. Use the same notation}

TRANS is transitivity of query containment. Follows directly from transitivity of subset \remark{Missing subject. Missing period at the end of the sentence.}
...
Here we use these sets to provide the same rewriting as in~\cite{guarnieri2016strong}, but for SQL. \remark{It is not the same rewriting. It even has different properties. It is ``inspired by the one in~\cite{guarnieri2016strong}''.}
%
...
\remark{Discuss the properties of these queries. Say clearly that one is the secure under- approx and the other is a secure over- approx of $Q$.
%
If you want, you can even discuss the relation between these queries and possible and certain answers.
}

Since we only consider tables or views from $T^\bot$ and $T^\top$, which, according to the access control policy, are readable to the user, it follows that any query constructed by the rewriting algorithm is secure. \remark{notation! Before you used $o^\top$ and $o^\bot$.}
%
...
%
\remark{The rewriting algorithm does not have anything to do with the result of $Q$. The queries approximate the result.}
...
\remark{There is an IMPORTANT missing bit from the algorithm. When you're rewriting the tables, if you are in a sub-query that is included in a NOT (for instance something like NOT EXISTS (SELECT * FROM ....) ), then you have to switch the rewriting from $Q^\top$ to $Q^\bot$ (and vice versa).
%
I recall we discussed this part a few times.
%
This corresponds to the inductive definition of $(\not \phi)^\top_u$ given in~\cite{guarnieri2016strong}.
%
I'm pretty sure the algorithm does not work properly without this fix.
%
This is something that MUST be addressed before submitting.
}
...
%
The mappings that are computed for the table \texttt{Programmers} before the rewriting algorithm look as follows: \remark{This is the only place where you use the term ``mapping''. Either you introduce it before, or just reuse the terminology you used when you introduced the $T^\top$ and $T^\bot$ notation.}
\begin{itemize}
	\item $T^\top = \lbrace \text{\texttt{ProgrammerOrClerk}, \texttt{ProgrammerOrHR}} \rbrace$ \remark{I guess $T^\top$ should be $\texttt{Programmer}^\top$}
...
%
Afterwards, the algorithm checks if the tuples returned by $Q^\top$ and $Q^\bot$ are the same, by executing the query  $Q^\top$ \texttt{EXCEPT} $Q^\bot$ and checking if it returns $\emptyset$. \remark{The algorithm does not check whether the tuples returned by $Q^\top$ and $Q^\bot$ are the same. It just checks whether \texttt{EXCEPT } is $\emptyset$. The fact that the results are the same follows from the reasoning below.}
%
...
%
Our implementation contains a subset of the introduced inference rules for containment, implementing all of them would result in a better under-approximation. \remark{Again, I wouldn't talk about the implementation since there is no working implementation at the moment. Talking about the implementation would deceive the reader a bit.}
%
