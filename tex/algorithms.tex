...
\remark{\cite{guarnieri2016strong} is not the first developing inference rules for containment. It is unclear why you're focusing on these rules.
%
I'd first introduce (maybe in background?) the algorithm from \cite{guarnieri2016strong}, and here I'd recall that query containment is a key part.
%
Then you can say that you generalize the rules in \cite{guarnieri2016strong}.
%
}
...
\remark{In a select query, both $T$ and $A$ are sequences I guess not sets.}
%
Figure~\ref{figure:algorithms:infrules} shows our inference rules.

\remark{We already discussed this yesterday. $Q$ must be any query (even stuff of the form $Q_1 \cup Q_2$). Just give a grammar defining all the queries we are interested in.}
...
\infer[\text{OR}]
	...
	{}
\]
\remark{Are $Q_1 \lor \psi$ and $Q_1 \land \psi$ queries?? }
...
\label{alg:algorithms:rewrite}
	...
	\ForEach{table $T$ referenced in $Q$}{
     ...
     \Return ($Q^\bot$, $Q^\top$)\;
\remark{Why not merging the two loops?}
\end{algorithm}
