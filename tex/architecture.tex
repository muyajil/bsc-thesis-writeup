...
%
\remark{Be careful. There is no running implementation of the new algorithm, so you shouldn't claim it as a result.}
...
%
The child process then will either authorize the request and become a \emph{Backend} or reject the request and exit. \remark{Is this ``authorize'' related to access control? What is a request?}
%
...
%
Each field in the decision data struct corresponds to a function that was migrated to the new module. \remark{This reads a bit weird. Is each field a pointer to a function?}
%
...
We also added a stack that, at any point of the execution of the database system, holds all queries that are not yet completed, meaning they were not yet interrupted by the access control mechanism nor have returned any tuples to the frontend application. \remark{Instead of talking about returning tuples isn't better to just say commands whose execution is not complete yet?}
%
...
%
The Backend process consists of one thread, and it is the only process that accesses the context stack, therefore it does not have to be thread-safe. \remark{What about multiple users connected to the database? Do they have one Backend (and one stack) each?}
% 
